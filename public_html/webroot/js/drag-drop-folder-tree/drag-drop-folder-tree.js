...
function JSDragDropTree()
{
  ...
  this.plusImage = 'uks_plus.gif';
  this.minusImage = 'uks_minus.gif';
  this.maximumDepth = 6;
...
Find left pos of a tree node
  */
  ...
  showHideNode : function(e,inputId)
  {
    ...
    //JSTreeObj.Set_Cookie('uks_expandedNodes', initExpandedNodes, 500);
    return false;
  }
  ,
  /* Initialize drag */
      ...
      while(li){
        if(li.id){
          ...
          if(numericID != '0') {
            ...
            if(li.parentNode.id!=this.idOfTree)saveString = saveString + numericParentID;
            else saveString = saveString + '0';

            saveString = saveString + '-';
            saveString = saveString + li.getElementsByTagName('a')[0].innerHTML;
          }
  ...
  {
    if(this.hasSubNodes(obj2.parentNode)) var message = obj2.innerHTML + '과(와) 하위메뉴를 삭제하시겠습니까?';
    else var message = obj2.innerHTML + '를 삭제하시겠습니까?';
  ...
  {
    this.__pushItem_step2(obj2, '새로운 카테고리');
  }
  ,
  __pushItem_step2 : function(obj, nodename)
  {
    ...
    li.id = 'node9999999999'; // No numeric id = new item
    ...
    var saveString = JSTreeObj.getNodeOrders();
    ...
    JSTreeObj.ajaxObjects[ajaxIndex].setVar("newIds", saveString);
    ...
    JSTreeObj.ajaxObjects[ajaxIndex].onCompletion = function() {
      JSTreeObj.__pushComplete(ajaxIndex,obj,li,aTag);
    }
    ; // Specify function that will be executed after file has been found
    JSTreeObj.ajaxObjects[ajaxIndex].runAJAX(); // Execute AJAX function
  }
  ,
  __pushComplete : function(ajaxIndex,obj,li,aTag)
  {
    /*
    if(this.ajaxObjects[ajaxIndex].response!='OK'){
      ...
      */
      li.id = 'node' + this.ajaxObjects[ajaxIndex].response;
      aTag.id = 'nodeATag' + this.ajaxObjects[ajaxIndex].response;
      aTag.href = '/webadm/coopers/index/' + this.ajaxObjects[ajaxIndex].response;
      aTag.target = 'contents';
  ...
  {
    if(this.ajaxObjects[ajaxIndex].response!='true'){
  ...
  initTree : function()
  {
    ...
    if(this.deleteAllowed || this.renameAllowed || this.addAllowed){
      try{
        ...
        if(this.addAllowed)menuModel.addItem(1,'카테고리 추가','','',false,'JSTreeObj.pushItem');
        if(this.deleteAllowed)menuModel.addItem(2,'카테고리 삭제','','',false,'JSTreeObj.deleteItem');
        if(this.renameAllowed)menuModel.addItem(3,'이름 변경','','',false,'JSTreeObj.renameItem');
        menuModel.init();

        var menuModelAddOnly = new DHTMLGoodies_menuModel();
        if(this.addAllowed)menuModelAddOnly.addItem(3,'카테고리 추가','','',false,'JSTreeObj.pushItem');
        menuModelAddOnly.init();

        var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
        if(this.deleteAllowed)menuModelDeleteOnly.addItem(4,'카테고리 삭제','','',false,'JSTreeObj.deleteItem');
        menuModelDeleteOnly.init();

        var menuModelRenameOnly = new DHTMLGoodies_menuModel();
        if(this.renameAllowed)menuModelRenameOnly.addItem(5,'이름 변경','','',false,'JSTreeObj.renameItem');
    ...
    initExpandedNodes = this.Get_Cookie('uks_expandedNodes');
    if(!initExpandedNodes) initExpandedNodes = ',0,';
