$(function(){
	...
	$('#btn-approve').click(function(){
		...
		if( window.confirm('선택한 게시물을 승인/비승인 상태로 변경 하시겠습니까?') == true ){
			$.ajax({
				url:'/webadm/board/multiapprove/' + $('#BoardBid').val(),
				type:'post',
				data:$('#form-list').serialize(),
				success:function(rst){
					if( rst == 'ok' ){
						$.jGrowl('승인상태가 변경 되었습니다.', { 
							theme: 'smoke',
							life: 1000,
		...
		$('#move').dialog({
			...
			buttons:{
				...
				'게시물 이동/복사':function(){
					...
					if( $('.move-mode:eq(0)').is(':checked') ) mode='move'; 
					else mode='copy';
					...
					$.ajax({
						...
						success:function(data){
							...
							if( $('.move-mode:eq(0)').is(':checked') ) method='이동'; 
							else method='복사';
